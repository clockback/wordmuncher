...
export function showEditSheetInterface(name) {
    hide([
...
export function hideEditSheetInterface() {
...
export function changeEditSheetName() {
    ...
    if (name == "" || name.length > 80) {
        ...
        hide([
            "message-edit-sheet-empty-name", "message-edit-sheet-long-name"
        ]);
...
export function keyDownOnEditSheetContainer(event) {
...
export function editSheetLoadMoreEntries(numberAlready) {
    numberAlready = document.querySelectorAll(
        "#edit-sheet-entry-table-rows>tr"
    ).length

    // Finds the search query
    var query = getById("edit-sheet-search-entries").value;
...
function processEditSheetLoadMoreEntries(request) {
    ...
    for (var i = 0; i < returnJSON['entries'].length; i ++) {
        ...
        newRow.onclick = clickEditSheetEntry;
...
export function editSheetSearchEntries() {
    // Finds the search query
    var query = getById("edit-sheet-search-entries").value;

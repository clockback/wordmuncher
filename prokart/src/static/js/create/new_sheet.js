...
export function hideNewSheetInterface() {
...
export function changeNewSheetName() {
...
export function keyDownOnNewSheetContainer() {
...
export function clickNewSheetEntry() {
...
export function newSheetLoadMoreEntries(numberAlready) {
    var numberAlready = document.querySelectorAll(
        "#add-sheet-entry-table-rows>tr"
    ).length

    // Finds the search query
    var query = getById("new-sheet-search-entries").value;
...
function processNewSheetLoadMoreEntries(request) {
    ...
    for (var i = 0; i < returnJSON['entries'].length; i ++) {
        ...
        newRow.onclick = clickNewSheetEntry;
...
export function newSheetSearchEntries() {
    // Finds the search query
    var query = getById("new-sheet-search-entries").value;

import {
    addClickEvent, assignBox, decorateFunction,
    disableAllTabbables, disableButtons, enableAllTabbables, enableButtons,
    getById, hide, openRequest, unhide
} from "../utils.js";
import {newEntryChooseSchema, showNewEntryInterface} from "./new_entry.js";
import {editEntryChooseSchema, showEditEntryInterface} from "./edit_entry.js";
import {chooseSchema, getSchemaAnswersAlt, pickNoSchema} from "./create.js";

export function showSchemasInterface(launchFrom, type) {
...
function processShowSchemasInterface(request, launchFrom, schemaName) {
    ...
    for (var i = 1; i < qualities.length && qualities[i][1] == 0; i ++) {
        ...
        columnTr.insertBefore(
            nextCell, columnTr.querySelector('td:last-child')
        );
    }
...
export function hideSchemasInterface() {
    ...
    for (i = 0; i < columnCells.length - 1; i++) {
        if (i == 0) {
            columnCells[i].querySelector(".quality-name").innerHTML = (
                "Column 1"
            );
        }
        else if (i == 1) {
            ...
            var rightArrow = columnCells[i].getElementsByClassName(
                "quality-right"
            );
    ...
    for (j = 0; j < rowCells.length - 1; j++) {
        ...
        rowCells[j].querySelector(".quality-name").innerHTML = "Row 1";
            ...
            var rightArrow = rowCells[j].getElementsByClassName(
                "quality-right"
            );
            if (rightArrow.length > 0) {
                rightArrow[0].remove();
...
export function keyDownOnSchemasContainer() {
...
export function changeSchemaName() {
    var nameEntry = getById('schema-name');
...
function swapQualities(origLeft, origRight) {
    ...
    if (newRight.getElementsByClassName("quality-left").length == 0) {
        newRight.insertBefore(
            generateQualityLeft(), newRight.firstElementChild
        );
    }
...
function typeSubschemaName() {
    if (
        this.value == "" || this.value.length > 80 || subschemaNameTaken(this)
    ) {
        this.style.backgroundColor = "pink";
    }
    else {
...
function processSaveSchema(request, name) {
    ...
    for (var p = 0; p < 2; p ++) {
        ...
        for (var i = 1; i < currentOptions.length; i ++) {
            if (
                name.toLowerCase() < currentOptions[i].innerHTML.toLowerCase()
            ) {
                break;
            }
        }
        ...
        var chooser = (
            schemaHeader == "Edit schema"
            ? editEntryChooseSchema : newEntryChooseSchema
        );
...
function reduceSubschema(rowsOrCols) {
    ...
    for (var i = 0; i < qualities.length - 1; i ++) {
        ...
        var nameAndId = [
            cellText.length == 0
            ? cell.getElementsByTagName("input").placeholder
...
export function showDeleteSchemaInterface() {
...
function processShowDeleteSchemaInterface(request, schemaName) {
    ...
    if (uses == 0) {
        ...
        schemaUsage.innerHTML = (
            `Currently, ${uses} solutions use this schema.`
        );
    }
...
export function changeDeleteSchemaName() {
